...
public class ConnectorResource extends ApplicationResource {
    ...
    private Map<String, String> extractPropertiesFromConfigurationStep(final ConfigurationStepConfigurationDTO configurationStep) {
        ...
        if (configurationStep.getPropertyGroupConfigurations() != null) {
            for (final org.apache.nifi.web.api.dto.PropertyGroupConfigurationDTO groupConfig : configurationStep.getPropertyGroupConfigurations()) {
                if (groupConfig.getPropertyValues() != null) {
                    properties.putAll(groupConfig.getPropertyValues());
                }
            }
        }
        return properties;
    ...
    /**
     * Prepares a connector for update by calling prepareForUpdate on the connector.
     *
     * @param id The id of the connector to prepare for update.
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{id}/prepare-update")
    @Operation(
            summary = "Prepares a connector for update",
            responses = {
                    @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = ConnectorEntity.class))),
                    @ApiResponse(responseCode = "400", description = "NiFi was unable to complete the request because it was invalid. The request should not be retried without modification."),
                    @ApiResponse(responseCode = "401", description = "Client could not be authenticated."),
            ...
            description = "This will invoke prepareForUpdate on the connector. The client can poll the connector endpoint to check if the preparation is complete.",
            security = {
                    @SecurityRequirement(name = "Write - /connectors/{uuid}")
            }
    )
    public Response prepareConnectorForUpdate(
            @Parameter(
                    description = "The connector id.",
                    required = true
            )
        ...
        return withWriteLock(
                ...
                (revision, connectorEntity) -> {
                    // prepare the connector for update
                    final ConnectorEntity entity = serviceFacade.prepareConnectorForUpdate(revision, id);
                    populateRemainingConnectorEntityContent(entity);
    ...
    /**
     * Finishes update process for a connector by calling finishUpdate on the connector.
     *
     * @param id The id of the connector to finish update for.
     * @param requestConnectorEntity A connectorEntity containing the revision.
     * @return A connectorEntity.
     */
    @POST
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{id}/finish-update")
    @Operation(
            summary = "Finishes update process for a connector",
            responses = {
                    @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = ConnectorEntity.class))),
                    @ApiResponse(responseCode = "400", description = "NiFi was unable to complete the request because it was invalid. The request should not be retried without modification."),
                    @ApiResponse(responseCode = "401", description = "Client could not be authenticated."),
            ...
            description = "This will invoke finishUpdate on the connector. The client can poll the connector endpoint to check if the finish process is complete.",
            security = {
                    @SecurityRequirement(name = "Write - /connectors/{uuid}")
            }
    )
    public Response finishConnectorUpdate(
            @Parameter(
                    description = "The connector id.",
                    required = true
            )
        ...
        return withWriteLock(
                ...
                (revision, connectorEntity) -> {
                    // finish the connector update
                    final ConnectorEntity entity = serviceFacade.finishConnectorUpdate(revision, id);
                    populateRemainingConnectorEntityContent(entity);
