...
export class HexViewer {
    ...
    @Input() set ref(ref: string) {
        this.contentViewerService.getBlob(ref).subscribe((blob: Blob) => {
            blob.arrayBuffer().then((buffer) => {
                const data = new Uint8Array(buffer);
                const lines: string[] = [];

                for (let i = 0; i < data.length && lines.length < HexViewer.MAX_LINES; i += 16) {
