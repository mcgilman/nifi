    ...
    public ellipsis(selection: any, text: string, cacheName: string) {
        ...
        if (!trimLength) {
            ...
            if (textLength > width) {
                ...
                selection.text(text + String.fromCharCode(8230));
                const textWithEllipsisLength = node.getSubStringLength(0, text.length);
                const ellipsisLength = textWithEllipsisLength - textLength;
                selection.text(text);

                // make some room for the ellipsis
                width -= ellipsisLength * 2;
